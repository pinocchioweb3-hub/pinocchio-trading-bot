...
def seed_known_decisions() -> None:
    ...
    add_decision(
        key="default_leverage_3x",
        title="預設槓桿是否由 15x 下調至 3x（小資保護）",
        detail=("3000U 陪跑藍圖建議：預設槓桿 15x→3x、<5000U 上限 5x。\n"
                "<b>影響實際倉位與保證金顯示</b>，故不單方面更動，等你決定。\n"
                "（風控金額閘門 v39 已上線且零行為改變；這項是「倉位大小」層）"),
        options=["同意下調為 3x", "維持 15x", "用其他倍數（請指定）"],
    )
