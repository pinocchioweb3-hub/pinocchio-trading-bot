...
GLOSSARY_VERSION = "1.2"  # 隨術語增修調版號（未來信任網頁/Agent 可據此判快取）
...
TERMS: tuple[Term, ...] = (
    ...
    Term("liquidation", "清算", "Liquidation",
         ...
         "flow"),
    Term("orderbook_wall", "掛單牆", "order-book depth",
         "聚合交易所掛單簿裡，買單(bid)與賣單(ask)的美元深度。買牆厚＝下方有人接、"
         "賣牆厚＝上方有人壓，是真實供需的代理。",
         "失衡＝(買牆−賣牆)/(買牆+賣牆)：偏正＝下方支撐較強，偏負＝上方壓力較重。",
         "掛單可瞬間撤換（spoofing 假牆很常見），是『此刻盤面氛圍』參考、非進場訊號；"
         "本工具僅作影子觀測，永不據此自動下單。",
         "flow"),
    Term("spot_perp_ratio", "現貨/合約量比", "spot/perp volume",
         "同一時間現貨主動成交量 ÷ 合約(永續)主動成交量。衡量一段行情是『真錢現貨』"
         "推動、還是『槓桿合約』推動。",
         ">1＝現貨主導（較扎實）；<1＝合約槓桿主導（較虛、易假突破）。用來幫忙分辨"
         "突破是否有現貨買盤支撐。",
         "永續量通常本來就大於現貨量，中性錨點為暫定基準、仍在校準；它是輔助濾鏡、"
         "非進場訊號，本工具僅作影子觀測，永不據此自動下單。",
         "flow"),
    Term("agg_cvd", "官方聚合CVD", "aggregated CVD",
         "CoinGlass 跨多家交易所聚合計算的累積成交量差，用來『對照校準』本工具自算的"
         "CVD 斜率（互為佐證，非取代）。",
         "斜率正＝主動買量淨多(偏多)、負＝主動賣量淨多(偏空)；和自算 CVD 同向時佐證較強。",
         "聚合 CVD 仍為估算且各源演算法有差異；它只是校準與盤面氛圍參考、非進場訊號，"
         "本工具僅作影子觀測，永不餵進下單用的訊號分數。",
         "flow"),
    # --- 結構與趨勢 ---
