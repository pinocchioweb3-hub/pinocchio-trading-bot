...
╔══════════════════════════════════════════════════════════════════════╗
║ ⛔ 手動專用腳本・嚴禁排程／嚴禁加入 run_bot.py worker 清單           ║
║                                                                      ║
║ 這是全專案唯一仍直接 import OKXExecutor（l4_execution.okx_executor）  ║
║ 的腳本。它只供「人為、單次、有人在看」的手動測試使用：               ║
║   • 預設且唯一可跑模式 = OKX 模擬盤（--demo，預設 True）。            ║
║   • 實盤路徑（--live）已在 main() 永久硬擋、直接退出（見檔尾）。     ║
║                                                                      ║
║ 紅線①：AI 永不自動動用真實資金。任何自動下單一律走                   ║
║   l4_execution.demo_trader（過 demo_guard 模擬盤正向證明），         ║
║   絕不經由本檔。請勿用 schtasks / Windows 工作排程器 / 任何 daemon    ║
║   迴圈呼叫本檔。常駐自動化 = run_bot.py 的受監督 worker，不含此檔。   ║
╚══════════════════════════════════════════════════════════════════════╝
