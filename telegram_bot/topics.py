...
class TopicRouter:
    ...
    def client(self, topic_key: str) -> TelegramClient:
        """取得綁定主題的 client；未設定 forum 時回傳原始 client（單 chat 行為）"""
        if not self.cfg:
            return self.base
        thread_id = self.cfg["topics"].get(topic_key)
        if thread_id is None:
            return self.base
        return self.base.for_topic(self.cfg["group_chat_id"], int(thread_id))
